...
async fn test_tray_list_operations() {
    ...
    if list_status == StatusCode::OK {
        ...
        for tray in trays {
            ...
            assert!(tray["created_at"].is_string(), "Each tray should have created_at");
            assert!(tray["last_updated"].is_string(), "Each tray should have last_updated");
            
            // Name can be null, so check if it's present and valid when not null
            if !tray["name"].is_null() {
                assert!(tray["name"].is_string(), "Tray name should be string when present");
            }
        }
    } else {
...
async fn test_tray_filtering_and_sorting() {
    ...
    if created_ids.is_empty() {
        ...
        if filter_status == StatusCode::OK {
            ...
            println!("Filtered trays by name=96-Well Plate: {} results", filtered_trays.len());
...
async fn test_tray_not_found() {
    ...
    let (status, _body) = extract_response_body(response).await;
    assert_eq!(status, StatusCode::NOT_FOUND, "Should return 404 for non-existent tray");
...
async fn test_tray_configuration_list_operations() {
    ...
    if list_status == StatusCode::OK {
        ...
        assert!(list_body.is_array(), "Tray configurations list should be an array");
        let tray_configs = list_body.as_array().unwrap();
        println!("Found {} tray configurations in the system", tray_configs.len());
        ...
        for config in tray_configs {
            ...
            assert!(config["created_at"].is_string(), "Each config should have created_at");
            assert!(config["last_updated"].is_string(), "Each config should have last_updated");
        ...
        if second_status == StatusCode::CREATED {
            ...
            if check_status == StatusCode::OK {
                if check_body["experiment_default"] == false {
                    println!("   ✅ Default configuration behavior working - first config no longer default");
                } else {
                    println!("   📋 Multiple default configurations allowed (or default logic not working)");
                }
            }
        } else {
...
async fn test_tray_configuration_not_found() {
    ...
    let (status, _body) = extract_response_body(response).await;
    assert_eq!(status, StatusCode::NOT_FOUND, "Should return 404 for non-existent tray configuration");
    ...
    if config_status == StatusCode::CREATED {
        ...
        if get_config_status == StatusCode::OK {
            ...
            if get_config_body["trays"].is_array() {
                ...
                println!("   Configuration loaded with {} assignments", assignments.len());
