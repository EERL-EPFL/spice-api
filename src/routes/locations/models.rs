...
pub struct Model {
    ...
    pub samples: Option<Vec<crate::routes::samples::models::Sample>>,
    #[sea_orm(ignore)]
    #[crudcrate(non_db_attr = true, default = None, list_model = false, create_model = false, update_model = false)]
    pub experiments: Option<Vec<crate::routes::experiments::models::Experiment>>,
}
...
async fn get_one_location(db: &DatabaseConnection, id: Uuid) -> Result<Location, DbErr> {
    let model = Entity::find_by_id(id)
        .find_with_related(crate::routes::samples::models::Entity)
        .all(db)
        .await?;
    ...
    let (project_color, project_name) = if let Some(project_id) = location_model.project_id {
        if let Some(project) = crate::routes::projects::models::Entity::find_by_id(project_id)
            .one(db)
            .await? {
    ...
    let experiments: Vec<crate::routes::experiments::models::Model> =
    ...
    for sample in samples {
        let treatments = crate::routes::treatments::models::Entity::find()
            .filter(crate::routes::treatments::models::Column::SampleId.eq(sample.id))
            .all(db)
            .await?;
