...
fn test_probe_yaml_config_structure() {
    ...
    for (name, sequence, excel_col, pos_x, pos_y) in expected_configs {
        assert_eq!(
            excel_col,
            sequence + 1,
            "Excel column should be sequence + 1 for {name}"
        );
        assert!(
            (0.0..=200.0).contains(&pos_x),
            "X position should be reasonable for {name}"
        );
        assert!(
            (0.0..=200.0).contains(&pos_y),
            "Y position should be reasonable for {name}"
        );
    }
...
fn test_probe_database_constraints() {
    ...
    let probe = models::Model {
        ...
        data_column_index: 1, // Excel column mapping for processing
    ...
    // Test key properties
    assert_eq!(probe.data_column_index, 1);
