...
async fn test_project_list_operations() {
    ...
    if list_status == StatusCode::OK {
        ...
        assert!(
            list_body.is_array(),
            "Project listing successful - Response should be an array"
        );
...
async fn create_test_project_with_params(
    ...
    if status == StatusCode::CREATED {
        ...
        assert!(
            body["id"].is_string(),
            "Project creation successful - Response should include ID"
        );
...
async fn test_project_retrieval(app: &axum::Router, project_id: &str) -> Option<serde_json::Value> {
    ...
    if get_status == StatusCode::OK {
        ...
        assert_eq!(
            get_body["id"], project_id,
            "Project retrieval successful - ID should match"
        );
...
async fn test_project_update(app: &axum::Router, project_id: &str, new_colour: &str) -> bool {
    ...
    match update_status {
        StatusCode::OK => {
            ...
            assert_eq!(
                update_body["colour"], new_colour,
                "Project update successful - Colour should be updated"
            );
            assert_eq!(
                update_body["note"], "Updated via test helper",
                "Project update successful - Note should be updated"
            );
            true
        }
        _ => false,
...
async fn create_multiple_test_projects(
    ...
    for i in 1..=count {
        ...
        match create_test_project_with_params(app, &name, colour, Some(&note)).await {
            ...
            Err(error) => {
                panic!("Batch project creation failed for {name}: {error}");
            }
        }
    }
...
async fn test_project_complete_lifecycle() {
    ...
    match project_result {
        ...
        Err(error) => {
            panic!("Project lifecycle test failed: {error}");
        }
    }
}
...
async fn test_project_helper_functions_consistency() {
    ...
    for (name, colour, note) in test_cases {
        ...
        match create_test_project_with_params(&app, &full_name, colour, note).await {
            ...
            Err(error) => {
                panic!("Project creation failed for {full_name}: {error}");
            }
        }
    }
}
